...
Запущенное приложение, по умолчанию, доступно по адресу http://localhost:8080 Адрес и порт можно поменять в файле pack/settings.txt

Приложение пишет логи в папку pack/logs Директория logs создается при первом запуске приложения.
...
Вычисление геозоны по точке реализовано не так, как предложено в [Задании](https://github.com/bugakov-sa/geo-service/blob/master/%D0%A2%D0%B5%D1%81%D1%82%D0%BE%D0%B2%D0%BE%D0%B5%20%D0%B7%D0%B0%D0%B4%D0%B0%D0%BD%D0%B8%D0%B5%20-%20%D0%B3%D0%B5%D0%BE%D1%81%D0%B5%D1%80%D0%B2%D0%B8%D1%81.pdf). В задании предложено брать в качестве координат ячейки целые части от широты и долготы. В реализации берется целая часть от широты, то есть, широта округляется в сторону 0. А долгота в реализации всегда округляется в меньшую сторону.
...
Эта фукнция убывает от 2 * Pi * R на экваторе до 0 на северном полюсе. Эта функция определяет закон убывания вероятности появления метки на широте lat. 
...
Можем построить такую сетку, у которой стороны всех ячеек будут, примерно, одинаковой длины. Обозначим эту длину К. Для это нужно на каждой параллели вводить свою ширину ячейки вдоль этой параллели. 

Как уже сказано выше, длина параллели широты lat: L(lat) = 2 * Pi * R * sin(lat).

На такой параллели умещается Int(L(lat) / K) ячеек, Int - взятие целой части.

Значит, ширина ячейки на данной параллели = L(lat) / Int(L(lat) / K)

Несложно понять, что поиск ячейки по такой сетке тоже можно выполнять за константное время.
